...
- (void)showViewSetup {
    ...
    } else if(self.showType == LFMessageAlertViewShowLeft) {
        self.center = CGPointMake(-[UIScreen mainScreen].bounds.size.width/2.0, self.bounds.size.height+NAVIGATION_BAR_HEIGHT);
    } else {
...
- (void)showViewAnimation {
    [UIView animateWithDuration:0.3 delay:0 options:(UIViewAnimationOptionCurveEaseInOut) animations:^{
        if (self.showType==LFMessageAlertViewShowTop) {
            ...
            self.center = CGPointMake(self.bounds.size.width/2.0+12, self.bounds.size.height+NAVIGATION_BAR_HEIGHT);
        }else {
