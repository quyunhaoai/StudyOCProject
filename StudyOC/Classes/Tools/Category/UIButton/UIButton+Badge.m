...
- (void)updateBadgeFrame
{
    ...
    if ([self.backgroundColor isEqual:self.badgeTextColor]) {
        self.badge.frame = CGRectMake(self.badgeOriginX, self.badgeOriginY, minWidth + padding, minHeight + padding);
        self.badge.layer.cornerRadius = (minHeight + padding) / 2;
        self.badge.layer.masksToBounds = YES;
    } else {
        self.badge.frame = CGRectMake(self.badgeOriginX+2, self.badgeOriginY+5*2, 5 + padding, 5 + padding);
        self.badge.layer.cornerRadius = (5 + padding) / 2;;
        self.badge.layer.masksToBounds = YES;
    }
...
- (void)updateBadgeValueAnimated:(BOOL)animated
{
    ...
    if ([self.badgeValue integerValue]>99) {
        self.badge.text = XYIntToString(99);
    } else {
        // Set the new value
