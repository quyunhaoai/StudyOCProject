...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    if ([self.delegate respondsToSelector:@selector(searchBarShouldEndEditing:)]){
        ...
        if (end) {
            if (@available(iOS 11.0, *)) {
                if (textField.text.length==0 && self.placeHolderCenter) {
                    [self setPositionAdjustment:UIOffsetMake((textField.frame.size.width-[self placeHolderTextWidth])/2, 0) forSearchBarIcon:UISearchBarIconSearch];
                }
            }
            if (self.clearBtnHidden) {
...
-(UITextField*)textField
{
    ...
    //        }
    //    }
//    searchField.layer.borderColor =[[UIColor colorWithRed:83.0f/255.0f green:83.0f/255.0f blue:83.0f/255.0f alpha:1.0f] CGColor];;
