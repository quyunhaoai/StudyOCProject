    ...
    if (!_beatTimer) {
        ...
        dispatch_source_set_event_handler(_beatTimer, ^{
            ...
            if (weakSelf.senBeatCount>TCP_MaxBeatMissCount) {
                ...
                [weakSelf.chatSocket writeData:beatData withTimeout:-1 tag:9999];
                NSLog(@"------------------发送了心跳------------------");
            }
        });
    }
...
{
    chatModel.bg_tableName = STBGFMDB_tableName;
    [chatModel bg_saveAsync:^(BOOL isSuccess) {
...
//#warning  - 注意 ...
    ...
    messageModel.bg_tableName = STBGFMDB_tableName;
