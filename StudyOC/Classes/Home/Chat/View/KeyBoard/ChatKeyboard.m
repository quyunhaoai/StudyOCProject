...
#define HOME_INDICATOR_HEIGHT 0
...
- (void)switchFaceKeyboard:(UIButton *)swtFaceButton
{
    ...
    if (swtFaceButton.selected) {
        ...
        //自定义键盘位移
        [self customKeyboardMove:SCREEN_HEIGHT - Height(self.frame)-HOME_INDICATOR_HEIGHT];
    }else{
...
- (void)audioButtonClick:(UIButton *)audioButton
{
    ...
    if (audioButton.selected) {
        ...
        [self customKeyboardMove:SCREEN_HEIGHT - defaultMsgBarHeight-HOME_INDICATOR_HEIGHT]; //默认高度 输入栏 49
    }else{
...
{
    if (self.msgTextView.text.length == 0) return;
    //回调
    if (_textCallback) {
...
- (void)keyboardResignFirstResponder:(NSNotification *)note
{
    ...
    //按钮初始化刷新
    [self reloadSwitchButtons];
    [self customKeyboardMove:SCREEN_HEIGHT - Height(self.messageBar.frame)-HOME_INDICATOR_HEIGHT];
}
