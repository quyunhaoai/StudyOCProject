...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    seletedChatModel.toUserID = @"123456";   //对方用户ID
    if (indexPath.row == 1) {
        seletedChatModel.contenType = @"groupChat";
    } else {
        seletedChatModel.chatType = @"userChat"; //聊天类型 , 群聊,单聊
    }
...
- (void)getMessages
{
    ...
    NSArray *tips = @[@"私聊今天晚上去喝酒。",@"群聊你好",@"私信通知。。"];
    for (NSInteger index  = 0; index < 3; index ++) {
        
        ChatModel *chatModel            = [[ChatModel alloc]init];
        ChatContentModel *chatContent   = [[ChatContentModel alloc]init];
        chatModel.content               = chatContent ;
        chatModel.nickName              = @"Jack";
        chatModel.lastMessage           = tips[index];
        chatModel.noDisturb             = index%3==0 ? @2 : @1;//免打扰状态  , 1为正常接收  , 2为免打扰状态 , 3为屏蔽状态
        chatModel.unreadCount           = @(index);//未读数
