...
//    self.navigationItem.title = @"国家代码";
    [self customNavBarWithTitle:@"    请选择国家和地区代码"];
    self.navTitleView.backgroundColor = kClearColor;
    self.navTitleView.titleLabel.textAlignment = NSTextAlignmentLeft;
    self.navTitleView.splitView.backgroundColor = COLOR_HEX_RGB(0xBDBDBD);
    [self creatSubviews];
}
...
- (void)creatSubviews{
    ...
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, NAVIGATION_BAR_HEIGHT, self.view.bounds.size.width, self.view.bounds.size.height-NAVIGATION_BAR_HEIGHT) style:UITableViewStylePlain];
