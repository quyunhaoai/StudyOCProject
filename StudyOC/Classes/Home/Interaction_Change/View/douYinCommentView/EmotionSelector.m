...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        for(int i=0;i<_data.count;i++) {
            ...
            if(_currentIndex == i) {
                pointView.backgroundColor = color_tipRed_FF0000;
            }else {
                pointView.backgroundColor = kGrayColor;
            }
        ...
        _send.titleLabel.font = FONT_10;
        [_send setTitle:@"发送" forState:UIControlStateNormal];
        [_send setTintColor:kWhiteColor];
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if([keyPath isEqualToString:@"attributedText"]){
        ...
        if(attributedString && attributedString.length > 0) {
            _send.backgroundColor = color_tipRed_FF0000;
            _send.enabled = YES;
        }else {
            _send.backgroundColor = ColorSmoke;
            _send.enabled = NO;
