...
- (void)setupUI{
    ...
    //===================================================================================================================
    [self.corverView setImageWithUrl:@"" placeholder:IMAGE_NAME(STSystemDefaultImageName) circleImage:NO completed:nil];
    TYTextContainer *container = [self createAttriDataWithText:@"#活动主题@小姐姐在直播，好像你看到了没有呢块来点击"];
    self.titleLabel.textContainer = container;
    [self.titleLabel mas_updateConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(container.attriTextHeight);
    }];
    NSString *playCount = [NSString stringWithFormat:@"123次播放"];
    self.playCountLabel.text = playCount;

    NSString *diggCount = [NSString stringWithFormat:@"123赞"];
...
- (TYTextContainer *)createAttriDataWithText:(NSString *)text{
    ...
    return [data createTextContainerWithTextWidth:Window_W/2.0 - 2 * kkPaddingNormal];
}
...
- (UIImageView *)corverView{
    if(!_corverView){
        _corverView = ({
            ...
            view.layer.borderColor = color_cellBg_151420.CGColor;
            view ;
        });
    }
    return _corverView;
