...
- (void)layoutSubviews{
    ...
    if(iPhoneX){
        if(self.smallType == KKSamllVideoTypeDetail){
            if(self.fullScreen){
                frame = CGRectMake(STATUS_BAR_HEIGHT, 0, self.bounds.size.width - STATUS_BAR_HEIGHT, self.bounds.size.height);
            }else{
                frame = CGRectMake(0, STATUS_BAR_HEIGHT, self.bounds.size.width, self.bounds.size.height - STATUS_BAR_HEIGHT);
            }
        }else{
            if(self.fullScreen){
                frame = CGRectMake(STATUS_BAR_HEIGHT, 0, self.bounds.size.width - STATUS_BAR_HEIGHT, self.bounds.size.height);
...
- (void)setFullScreen:(BOOL)fullScreen{
    ...
    if(_fullScreen){
        ...
        CGFloat width = MIN(Window_W,Window_H);
...
- (KKCacheSliderView *)slider{
    if(!_slider){
        _slider = ({
            ...
            view.value = 0.0;
            view.cacheColor = KKColor(175, 169, 159, 1.0);
            view.cachaValue = 0.0;
            view.backgroundColor = [UIColor clearColor];
            view.minimumTrackTintColor = COLOR_HEX_RGB(0xB27100);
            view.maximumTrackTintColor = [COLOR_HEX_RGB(0xD5D5D5) colorWithAlphaComponent:0.3];
